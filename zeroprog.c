//installed libusb-1.0_dev
//added the libusb***.so lib to linker options
...
#define VERSION "1.1"   //edit initial string DaveB
...
static void show_help(char *progname) {
	...
	printf("  -v, \t\tchange the vendor ID to the default 0x%04x\n", ZP_VID);
	printf("  -p, <PID>\tchange the product ID, e.g. 0x700d\n");
	for (i=0; i < sizeof(zeroplus_models)/sizeof(zeroplus_models[0]); i++)
		printf("\t\t0x%04x %s\n", zeroplus_models[i].pid, zeroplus_models[i].model_name);
	printf("  -m, \"string\"\tchange the manufacturer string\n");
	printf("  -o, \"string\"\tchange the model name string\n");
	printf("  -s, \"string\"\tchange the serial string\n");
	printf("  -b, filename\tdump the entire EEPROM to file\n");
	printf("  -r, filename\trestore the entire EEPROM from a file\n");
...
int main(int argc, char *argv[])
{
	...
	printf("\nZeroplus EEPROM re-programmer V%s", VERSION);
    printf("\n\nWait...\n");
